...
* 可以通过vim编辑 输入命令 vim /etc/yum.repos.d/jenkins.repo
* （按键盘i 键进行将下面内容赋值到文件中 在按Esc键退出, 最后按shift + :  输入wq(保存且退出)即可）
...
因为jenkins需要到github上录取我们最新的代码, 然后进行以下命令执行
...
* 在下面的地址中 `cat /var/lib/jenkins/secrets/initialAdminPassword`      在终端中输入该命令会获取到密码，最后输入到网页http://8.134.60.235:8080/中
...
改完配置过够需要重新启动nginx    命令为 systemctl restart nginx

#### 手动部署

- 在服务器里面创建一个文件夹  mkdir xxx(文件夹的名字)
- 在服务器中打开上面创建出来的文件夹 ，然后将本地打包的文件拖到该文件夹下面
